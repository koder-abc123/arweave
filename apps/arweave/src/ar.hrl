...
-record(block, {
	...
	%% A list (a collection) of {address, balance, last TX ID} tuples.
	%%
	%% Wallet lists are reconstructed on the receiving side and stored in the separate files.
	%%
	%% The field was originally a list (hence the name). In 2.2 we switched to a tree,
	%% closely resembling Patricia Merkle trees.
	...
	%%
	%% In the block shadows and in the /block/[hash] API endpoint the field stores the
	%% root hash of the tree.
	wallet_list = ar_patricia_tree:new(),
	%% A field for internal use, not serialized. Added to avoid hashing the wallet list
	%% when the hash was known before the list was reconstructed. The field is only
	%% helpful for blocks before 2.2, because the new Patricia tree remembers its
	%% root hash anyway. Thefore, the field needs to be removed some time after 2.2.
